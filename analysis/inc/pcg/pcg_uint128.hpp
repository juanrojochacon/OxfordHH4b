...
inline bitcount_t flog2(uint32_t v) {
    ...
    static const uint8_t multiplyDeBruijnBitPos[32] = {0,  9,  1,  10, 13, 21, 2,  29, 11, 14, 16,
...
inline bitcount_t trailingzeros(uint32_t v) {
    static const uint8_t multiplyDeBruijnBitPos[32] = {0,  1,  28, 2,  29, 14, 24, 3,  30, 22, 20,
...
template <typename UInt, typename UIntX2> class uint_x4 {
  ...
  public:
    ...
    typedef typename std::conditional<std::is_same<uint64_t, unsigned long>::value,
                                      unsigned long long, unsigned long>::type uint_missing_t;
