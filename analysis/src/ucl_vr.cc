...
UCLVRAnalysis::UCLVRAnalysis(std::string const& sampleName):
Analysis("ucl_vr", sampleName)
{
// ********************* Histogram settings******************
...
void UCLVRAnalysis::Analyse(bool const& signal, double const& weightnorm, finalState const& fs)
{
	...
	if( higgs[0].pt() < pt_dijet_ucl || higgs[1].pt() < pt_dijet_ucl ) // Was bugged, to higgs1 in both cases
	{
...
void UCLVRAnalysis::JetCluster_SmallVR(finalState const& particles, std::vector<fastjet::PseudoJet>& bjets, double& event_weight)
{
...
bool UCLVRAnalysis::BTagging( fastjet::PseudoJet const& jet ) const
{
