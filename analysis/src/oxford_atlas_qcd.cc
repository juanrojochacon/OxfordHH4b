...
OxfordAtlasQcdAnalysis::OxfordAtlasQcdAnalysis(runCard const& run, sampleCard const& sample, int const& subsample, int const& nBTag):
Analysis("atlas_qcd", run, sample, subsample),
...
void OxfordAtlasQcdAnalysis::HiggsFill(fastjet::PseudoJet const& H0,
                                         fastjet::PseudoJet const& H1,
                                         std::string const& analysis,
                                         std::string const& ntagcat, 
                                         size_t const& cut, 
                                         double const& weight)
{
  if (H0.pt() < H1.pt())
    std::cerr << "HiggsFill WARNING: pT ordering incorrect! "<<analysis<<"  "<<cut<<"  "<<ntagcat<<"  "<<H0.pt() << "  "<<H1.pt()<<std::endl;
...
void OxfordAtlasQcdAnalysis::BoostFill( fastjet::PseudoJet const& H,
                                          std::string const& analysis,
                                          std::string const& ntagcat, 
                                          size_t const& cut, 
                                          double const& weight )
...
void OxfordAtlasQcdAnalysis::BoostFill( fastjet::PseudoJet const& H0,
                                          fastjet::PseudoJet const& H1,
                                          std::string const& analysis,
                                          std::string const& ntagcat,
                                          size_t const& cut, 
...
{
  if (H0.pt() < H1.pt())
    std::cerr << "BoostFill WARNING: pT ordering incorrect! "<<analysis<<"  "<<cut<<"  "<<ntagcat<<"  "<<H0.pt() << "  "<<H1.pt()<<std::endl;
