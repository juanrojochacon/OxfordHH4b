...
void AMCAnalysis::Analyse(bool const& signal, double const& weightnorm, finalState const& fs)
{
	...
	const std::vector<fastjet::PseudoJet> jets_akt_80  = sorted_by_pt( rap_sel(cs_akt.inclusive_jets( 80.0  ))  );
	const std::vector<fastjet::PseudoJet> jets_akt_100 = sorted_by_pt( rap_sel(cs_akt.inclusive_jets( 100.0 ))  );
