...
\subsection{Object Definition and Selection}

All jets are clustered using all final state particles after the parton shower (but before hadronisation) as input. 

Small-$R$ jets are anti-$k_T$ $R=0.4$ jets. They are required to have $p_T > 40$~GeV and $|\eta|<2.5$.

Small-$R$ subjets are anti-$k_T$ $R=0.3$ jets. They are required to have $p_T > 50$~GeV and $|\eta|<2.5$.

Large-$R$ jets are anti-$k_T$ $R = 1.0$ jets with $p_T > 200$~GeV and $|\eta|<2.0$ (the smaller pseudorapidity is an experimental requirement related to the trackjet-based calibration). They are also required to be mass-drop tagged using the FastJet default parameters: $\mu = 0.67$ and $y_{\textrm{cut}}= 0.09$.
...
\subsection{Intermediate Topology}
...
that is selected as a Higgs boson candidate. The second Higgs boson candidate is reconstructed from the two small-$R$ jets that minimise the difference $|m_{fj} - m_{j1j2}|$.

\subsection{Higgs Mass Window Cut}

All Higgs candidates are required to meet the following Higgs mass window cut: $|m_{H} - 125| < 40$~GeV

[THE INFORMATION BELOW HAS NOT BEEN UPDATED YET]
