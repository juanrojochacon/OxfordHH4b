...
around the nominal Higgs boson mass $m_h\simeq 125$
GeV~\cite{Aad:2015zhl}.
%
...
structure.
%
As discussed in Sect.~\ref{sec:detectormodeling}, the
width of the Higgs mass peak arises both from QCD effects
like ISR and out-of-cone radiation, as well
as from the smearing applied to final state particles.
%
While the Higgs mass window requirements
in the cut-based analysis are relatively loose, the
information of the different shape of the $m_{h}$
distribution will still be fully exploited by the MVA.
%
...
enhanced, for example due to
of certain $d=6$ EFT operators~\cite{Azatov:2015oxa}.
%
...
discrimination: we will verify this with the MVA.
%
...
for signal events has a kinematical
cut-off at $m_{hh}^{\rm min}=250$ GeV, which is modified due to both
the parton shower and to detector resolution effects.
%


Finally, in Fig.~\ref{fig:pthh} we show the transverse momentum of
...
%
Note that in our LO simulation the $p_T^{hh}$ distribution is generated
purely
by the parton shower - a more refined treatment would require
...
\subsection{The impact of pile-up}
\label{sec:pileup}

The results shown so far did not accounted for the
effects of PU.
%
...
%
While the latter is closer to the HL-LHC forecasts, since it
is beyond the scope of this paper to fully optimize
PU subtraction, we will adopt $\la n_{\rm PU}\ra=80$ as our baseline.
...
%
This recently developed method has been shown to improve~\cite{Cacciari:2014gra} on the
performance of the commonly used area-based subtraction~\cite{Cacciari:2009dp}.
%
...
%
We will find that
{\tt SoftKiller} exhibits a good
performance for the PU subtraction in the present case.
%
...
distribution.
%
The performance of SK is even better in the boosted category,
...
after the {\tt SoftKiller} subtraction are rather moderate,
...
These results validate our strategy for PU subtraction and indicate
that, while there is some distortion in the shape of kinematical
distributions as compared to the case without PU, it is certainly
not enough to wash out the most important differences between
signal and background events.
