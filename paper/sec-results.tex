...
QCD multi-jet background is taken into account (leaving out the 
the $2b2j$ and $4j$ components).
%
First we present the results in an environment with no pile-up,
followed by the corresponding analysis including PU effects.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    \item {\bf C1c}: jets are required to satisfy their associated
...
In Table~\ref{tab:cutflow_noPU_1} we also  provide the signal over
      ...
      analysis is degraded by about a factor of two due to the contribution
...
\begin{table}[t]
  ...
  \vspace{0.5cm}
    ...
    \caption{\small The cross-sections, in femtobarns,
      ...
      significances are calculated for a total integrated luminosity of $\mathcal{L}=3$ ab$^{-1}$.
      %
      The different steps of the cut-flow are summarized
      in Table~\ref{tab:cutflowdetails}.
...
specially in the resolved category, but this enhancement is compensated for by the
corresponding
...
quantify
the impact of light and charm jet mistags. 
%
...
mass $m_{hh}$ of the
    ...
    for the two distributions, albeit with some interesting
...
\ee
with $n^{\rm (b-jet)}_j$ being the fraction of events satisfying all the selection cuts,
...
\frac{{\rm EFF}_{\rm b-tag} \lc 2b2j\rc}{{\rm EFF}_{\rm b-tag} \lc 4b\rc}=
  ...
  in the resolved category before $b$-tagging, shown
  in Table~\ref{tab:cutflow_noPU_1}.
    %
